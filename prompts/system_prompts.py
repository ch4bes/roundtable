...
- Identify ALL clusters in the matrix: scores >{threshold} are strongly aligned, {threshold - 0.3:.2f}-{threshold} are somewhat aligned, <{threshold - 0.3:.2f} are different perspectives
...
CONSENSUS RULE:
- If ALL clusters agree on the MAIN ANSWER → consensus_assessment: "REACHED"
- If clusters fundamentally disagree on MAIN ANSWER → consensus_assessment: "NOT REACHED"
- Peripheral disagreements (different examples) do NOT prevent consensus on the main answer

OUTPUT FORMAT:
Write your cluster analysis and reasoning in free-form prose. Then close with a single
JSON code block containing the structured result. Do NOT output any text after the block.

```json
{{
  "individual_summaries": {{
    "<MODEL_NAME>": ["<main point>", "<supporting point>"],
    "<MODEL_NAME>": ["<main point>"]
  }},
  "agreement_analysis": "<paragraph summarising where participants agree and disagree>",
  "consensus_assessment": "REACHED",
  "confidence": "HIGH"
}}
```

consensus_assessment MUST be exactly "REACHED" or "NOT REACHED".
...
"""
    ...
    def template(responses: list[dict[str, str]], round_num: int) -> str:
        ...
        return (
            ...
            "Analyse the responses and output your structured result as a JSON block "
    ...
    def _format_similarity_matrix(matrix: np.ndarray, model_names: list[str]) -> str:
        ...
        header = (
            "| " + " | ".join(name.center(col_width) for name in short_names) + " |"
        )
    ...
    def template_with_similarity_matrix(
        ...
        matrix_table = ModeratorPrompt._format_similarity_matrix(
...
Use the similarity matrix to identify clusters and determine consensus. \
Output your structured result as a JSON block at the end (format in the system prompt)."""
...
Provide your updated position."""
    ...
    def with_context(
        ...
        context_text = "\n".join(
            f"{r['model']}: {r['content']}" for r in context_responses
        )
        return f"{prompt}\n\nRESPONSES:\n{context_text}\n\nModel {model_position}/{total_models} round {round_num}."
